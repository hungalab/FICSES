...
// Zeus to FICSES memcpy parameters
...
int ficses_register_access(int BOARD_ID, int command, int address, int payload, int ficses_channel, int * read_data) {
	...
	if (command ==REG_RD)
		ficses_read_128KB(READ_BINARY_FILE, ficses_channel, read_data);
...
// User function: send data_countx32bit data via FICSES
int user_write_data(float *array, int BOARD_ID, int data_count, int ficses_channel) {

    float 	data1; 			// 32bit: [31:0]
    float 	data2; 			// 32bit: [127:32]
    float 	data3; 			// 32bit: [159:64]
    float 	data4; 			// 32bit: [127:96]
	...
	if (data_count%PACKETMAX ==0){	
		...
		array = (float *)realloc(array, sizeof(float)*tmp_data_count);
	}
